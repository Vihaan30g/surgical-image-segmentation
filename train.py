...
# Checkpointing
# --------------------------------------------------------------------------
def save_checkpoint(model, optimizer, epoch: int, best_val_dice: float,
                     path: str = config.CHECKPOINT_LATEST):
    # Scheduler state is intentionally NOT saved — see build_scheduler()
    # below. A saved CosineAnnealingLR state bakes in whatever T_max was
    # set at save time; if you later change config.NUM_EPOCHS and resume,
    # the scheduler is instead rebuilt fresh each run to match.
    checkpoint = {
...
def load_checkpoint_if_exists(model, optimizer, path: str = config.CHECKPOINT_LATEST):
...
def build_scheduler(optimizer, start_epoch: int, total_epochs: int = config.NUM_EPOCHS):
    """Fresh CosineAnnealingLR positioned correctly for resuming, using the
    CURRENT config.NUM_EPOCHS as T_max — lets you bump NUM_EPOCHS and
    resume without the LR curve behaving like it already finished."""
    return torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=total_epochs, last_epoch=start_epoch - 1
    )
...
def main():
    ...
    # --- Datasets (clip-level split — see splits.py, generated by
    #     generate_clip_split.py — instead of the old whole-video split,
    #     which starved several rare classes from train/val entirely) ---
    train_dataset = CholecSegDataset(
        root_dir=config.DATA_ROOT,
        clip_list=splits.TRAIN_CLIPS,
        frame_step=config.TRAIN_FRAME_STEP,
        transforms=get_train_transforms(),
    )
    val_dataset = CholecSegDataset(
        root_dir=config.DATA_ROOT,
        clip_list=splits.VAL_CLIPS,
    ...
    if config.CLASS_WEIGHTS is not None:
        print(f"Using per-class loss weights: {config.CLASS_WEIGHTS}")
        criterion = ComboLoss(class_weights=config.CLASS_WEIGHTS)
    else:
        print("WARNING: config.CLASS_WEIGHTS is None — training with unweighted "
              "loss. Rare classes will likely stay near 0 dice. Run "
              "compute_class_weights.py and set CLASS_WEIGHTS.")
        criterion = ComboLoss()
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=config.LEARNING_RATE, weight_decay=config.WEIGHT_DECAY
    )

    # --- Auto-resume (model + optimizer only; scheduler rebuilt below) ---
    start_epoch, best_val_dice = load_checkpoint_if_exists(model, optimizer)

    scheduler = build_scheduler(optimizer, start_epoch, total_epochs=config.NUM_EPOCHS)
    print(f"Scheduler: cosine over {config.NUM_EPOCHS} total epochs, "
          f"resuming at position {start_epoch} (current LR: {optimizer.param_groups[0]['lr']:.2e})")
    ...
    for epoch in range(start_epoch, config.NUM_EPOCHS):
        ...
        if val_metrics["mean_dice"] > best_val_dice:
            ...
            save_checkpoint(model, optimizer, epoch, best_val_dice, path=config.CHECKPOINT_BEST)

        save_checkpoint(model, optimizer, epoch, best_val_dice, path=config.CHECKPOINT_LATEST)
